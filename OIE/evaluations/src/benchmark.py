...
class Benchmark:
    ...
    def compare(self, gold, predicted, matchingFunc, output_fn, error_file=None):
        ...
        for sent, goldExtractions in gold.items():
            ...
            for predictedEx in [
                x
                for x in predictedExtractions
                if (("matched" not in x) or (output_fn not in x["matched"]))
            ]:
                # Add false positives
                y_true.append(0)
                y_scores.append(predictedEx["confidence"])
    ...
    def prCurve(y_true, y_scores, recallMultiplier):
        ...
        return ((precision_ls, recall_ls), optimal)
...
if __name__ == "__main__":
    ...
    other_p.read(s_fn)
    b.compare(
        predicted=other_p.oie, matchingFunc=matchingFunc, output_fn=out_path, error_file=error_fn
    )
